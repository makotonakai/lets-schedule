...
func TestGetMeetingByIdFirst(t *testing.T){
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `meetings` WHERE meetings.id = ?")).
		WithArgs(userId).
	...
	result, err := models.GetMeetingById(gormDB, userId)
