...
func TestGetNotConfirmedMeetingsForHostByUserIdFirst(t *testing.T){
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `meetings` inner join participants on participants.meeting_id = meetings.id inner join users on users.id = participants.user_id WHERE (participants.user_id = ? AND participants.is_host = ?) AND (meetings.is_confirmed = ? AND meetings.all_participants_responded = ?)")).
		WithArgs(userId, 1, 0, 1).
	...
	result, err := models.GetNotConfirmedMeetingsForHostByUserId(gormDB, userId)
...
func TestGetNotConfirmedMeetingsForHostByUserIdNil(t *testing.T){
