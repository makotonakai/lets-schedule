...
func TestGetCandidateTimeByMeetingIdSuccess(t *testing.T){
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `candidate_times` WHERE candidate_times.meeting_id = ?")).
		WithArgs(meetingId).
	...
	result, err := models.GetCandidateTimeByMeetingId(gormDB, meetingId)
